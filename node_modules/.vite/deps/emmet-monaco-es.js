...
function doComplete(monaco, model, position, syntax, emmetConfig) {
  ...
  if (!extractedValue)
    return;
  const { abbreviationRange, abbreviation: abbreviation2, currentLineTillPosition, filter } = extractedValue;
  ...
  const createExpandedAbbr = (syntax2, abbr) => {
    if (!isAbbreviationValid(syntax2, abbreviation2))
      return;
    try {
  ...
  return completionItems.length ? { suggestions: completionItems, incomplete: true } : void 0;
}
...
function getTokenizationEnv(model) {
  ...
  let _tokenization = (
    ...
    model._tokenization || // monaco-editor >= 0.35.0
  ...
  if (!_tokenization || !_tokenizationStateStore) {
    ...
    const _tokens = (
      ...
      _t.grammarTokens || // monaco-editor 0.52.0
      _t._tokens || // monaco-editor >= 0.53.0
  ...
  const _tokenizationSupport = (
    ...
    _tokenizationStateStore.tokenizationSupport || // monaco-editor <= 0.31.0
...
function registerProvider(monaco, languages, syntax, options) {
  ...
  const providers = languages.map((language) => monaco.languages.registerCompletionItemProvider(language, {
    ...
    provideCompletionItems: (model, position) => isValidLocationForEmmetAbbreviation(model, position, syntax, language, options) ? doComplete(monaco, model, position, syntax) : void 0
  }));
  return () => {
...
//# sourceMappingURL=emmet-monaco-es.js.map
